...
for expected in image_0_x86-64.raw image_0_x86-64.qcow2 image_0_x86-64.efi \
    ...
    image_0_x86-64.spdx.json image_0_x86-64.cdx.json image_0_x86-64.pkgdb \
...
# A package database is published as the directory it occupies, so what it holds is the package
# system's own layout; rpm's is the single file its capture driver writes.
if [ ! -f "$directory/image_0_x86-64.pkgdb/rpmdb.sqlite" ]; then
    echo "release: the published package database holds no rpmdb" >&2
    missing=1
fi
