...
# Whether the configured cache takes uploads, which the execution platform reads back.
CACHE_UPLOADS = "cache-uploads"
...
BLOCK_NOTE = "# Buck reads the version components, mount ignores and cache settings; Tine reads its TOML."
...
def project_settings(root: Path) -> dict[str, dict[str, object]]:
    ...
    for path in (root / CONFIG, root / LOCAL_SETTINGS):
        ...
        for section, table in settings.items():
            values = object_table(table, f"[{section}] in {path}")
            if section == cache_shim.SECTION and path.name == CONFIG:
                if local_only := sorted(set(values) & cache_shim.LOCAL_ONLY):
                    named = ", ".join(local_only)
                    fail(f"[{section}] in {path} sets {named}, which belong in {LOCAL_SETTINGS}")
...
def render_local_config_block(
    root: Path, ignores: list[str], cache: cache_shim.CacheSettings | None = None
) -> list[str]:
    ...
    if isinstance(derived, str):
        ...
        own |= {f"version-{key}": value for key, value in derived.items()}
    if cache is not None:
        # Buck2 never asks the shim whether it may upload, so the execution platform reads it here.
        own[CACHE_UPLOADS] = "true" if cache.accepts_uploads else "false"
    if own:
        lines += ["", f"[{SECTION}]"] + [f"{key} = {value}" for key, value in own.items()]
    if ignores:
...
def refresh_local_buckconfig(
    root: Path, ignores: list[str], cache: cache_shim.CacheSettings | None = None
) -> None:
    """Refresh the version data, mount ignores and cache address Buck consumes from its local config."""
    local = root / LOCAL
    write_if_changed(
        local, merge_generated_config_block(local, render_local_config_block(root, ignores, cache))
    )
...
def _pinned_binary(
    ...
    binary = cache_home() / "tine" / tool / sha256 / tool
    if binary.is_file():
        return binary
    if not fetch:
        return None
...
def buck_command(argv: list[str]) -> None:
    ...
    if cache is not None:
        ...
        # would win silently and leave Tine starting a shim nothing talks to.
        if RE_CLIENT in config:
...
def cache_status_command(root: Path, arguments: list[str]) -> None:
    ...
    configured = cache_shim.settings(project_settings(root), root, SETTINGS)
...
def main(argv: list[str]) -> None:
    ...
    if name == "buck":
        ...
        cache_status_command(project_root(cwd()), rest)
    elif name == "init":
