...
# Not `[buck2]`, which is Buck2's own to define keys in, nor `[tine.buck2]`, which `audit config`
# cannot address.
SECTION = "tine"
BUCK2 = "buck2-"
...
BLOCK_NOTE = "# `tine mount` writes here; edit a mount with that command, not by hand."
...
def project_config(root: Path) -> dict[str, dict[str, str]]:
    ...
    """
    config: dict[str, dict[str, str]] = {}
    # Buck2 lists that directory with `lstat`, so a symlink in it is not configuration it reads.
...
def reads_back(root: Path, target: str, source: str) -> bool:
    """Check that the buckconfig parser reads a serialized mount unchanged.

    Buckconfig also interprets `[` as a section and `<` as an include. Parsing the serialized line catches
    target names that would otherwise disappear on the next read.
    """
    config: dict[str, dict[str, str]] = {}
    _fill(_logical([f"[{MOUNTS}]", f"{target} = {source}"]), config, "", root)
    return config.get(MOUNTS) == {target: source}
...
def mountable(
    ...
    *,
    ...
    """Validate a generated or hand-written mount declaration.
    ...
    path = Path(target)
    if not round_trips(source):
        raise fail(f"mount {target}: source {source!r} cannot be represented in .buckconfig.local")
    if not round_trips(target) or not reads_back(root, target, source):
        raise fail(f"mount {target!r}: target cannot be represented in .buckconfig.local")
...
def declared_mounts(root: Path) -> dict[str, str]:
    ...
    """
    mounts: dict[str, str] = {}
    for target, source in sorted(generated(root / LOCAL).get(MOUNTS, {}).items()):
...
def generate(root: Path, mounts: dict[str, str]) -> list[str]:
    lines = [BLOCK_BEGIN, BLOCK_NOTE]
    if mounts:
        lines += ["", f"[{MOUNTS}]"] + [f"{target} = {source}" for target, source in sorted(mounts.items())]
...
def _refresh(root: Path, mounts: dict[str, str]) -> None:
    local = root / LOCAL
    write_if_changed(local, merge(local, generate(root, mounts)))


def refresh(root: Path) -> None:
    with ConfigLock(root):
        _refresh(root, declared_mounts(root))
...
def buck2(config: dict[str, dict[str, str]], cell: Path, *, fetch: bool = True) -> "Path | None":
    ...
    The pin is this cell's, overridable key by key in `[tine]`. Cached by its own hash, so a bumped
    pin lands beside the old one and going back finds it there. Without `fetch`, None.
    """
    platform = _platform()
...
def project_buckconfig(source: Path, cell: str) -> str:
    ...
    Copied rather than written afresh, so what a project has no say in cannot fall out of step with
    the cell. Only the cells are the project's own, and the detector that platforms its targets.
    """
    lines = [
...
def mount(root: Path, arguments: list[str]) -> None:
    ...
    if args.verb == "list":
        declared = generated(root / LOCAL).get(MOUNTS, {})
