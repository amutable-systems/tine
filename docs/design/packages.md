...
## Branch/Directory Layout
 ...
 - The "main" branch contains the packages from which we actually build the downstream OS.
   * It mirrors the per-distro-release `packages/` structure from the `upstream-rpm` branch.
   * This allows us to build downstream images from multiple distributions in the future. We value having that capability. We won't actually *do* it at first to avoid multiplying the maintenance overhead, but eventually we may want to build e.g. a CentOS 10 based image.
   ...
   * Might contain downstream specific packages, like a management CLI, with no upstream dist-git. I.e. `upstream-rpm` branch does *not* have that package. These live in `packages/aos/{latest,lts,...}/`_srcpkg_. (Note: We will try hard to just have one "latest" release stream -- but we could have more than one if we really have to)
...
To actually build an image, the selection needs to happen in some "build configuration". E.g. production could define "take packages from fedora/f44 by default, but take our own management CLI and systemd (our fork) from aos/latest". A second "future" configuration defines "take everything from rawhide, plus our own management CLI from aos/latest". This can take the form of a yaml, or buck rules, or something else, TBD.
...
## Design principles
 ...
 - Local modifications don't modify `%changelog` and bump `Release:` in our own sub-namespace; the concrete conventions developers follow are spelled out in [importer.md](../user/importer.md)
 ...
 * `check`: Walk through all commits (or all since the given ref, `origin/main` in PRs) of the current branch (usually `main` or a temporary developer or package update PR branch). For each commit that touches `packages/`:
   ...
   - if it does *not* have an `X-Upstream-Commit:` and *only* changes `srcpkg.json` metadata, check that the package uses `%autorelease` and commit has `X-Rebuild:`. Every other case of local metadata modification has to come from a sourceful modification or no-change rebuild which bumps `Release:`. Two legitimate exemptions:
     ...
     * commits that discard our delta (see the bump rules above): they take a modification away instead of making one, so they equally have no `Release:` bump and no `X-Rebuild:` to show — and need none, as their release is the import's, established by their tree
...
The `buildroot_only_packages` branch property lists these packages: a listed edge survives the cycle-breaking, so it introduces a gcc → glibc dependency edge and hence gcc builds against *our* glibc build. The reverse glibc → gcc edge stays dropped as per the general rule above. When and how to add an entry, and the `_properties.json` format, are documented in [importer.md](../user/importer.md).
