...
`aos` is this document's placeholder for the downstream OS's own short identifier. It appears in the `packages/aos/` namespace and as the `%dist` suffix; a downstream project substitutes its own (`NATIVE_DIST` in `tine/tools/importer`, and the `dist` attribute in `tine/package_system/rpm/rules.bzl`).
