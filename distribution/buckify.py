...
Runs on the host (a python_bootstrap_binary), invoked via `buck2 run
@tine//distribution:buckify -- --catalog-dir <dir> --buck2 <buck2>`. It does NOT
resolve packages itself; it drives the per-distribution, per-format resolvers:

  1. `buck2 uquery` to find the `catalog//:<distribution>.buckify` resolve targets (each a
  ...
  2. for each, nest `buck2 run <target> -- resolve --distribution <distribution> --catalog-dir
...
Nested buck2 is supported: a `buck2 run` target executes with cwd = the invocation
dir and inherits PATH + BUCK_ISOLATION_DIR, so a child `buck2` reuses the same
daemon (verified against the buck2 source). The buck2 binary is passed in (--buck2)
because this is a python child, not buck2 itself (buck2 finds *itself* via
...
def _resolve_targets(buck2: str) -> list[str]:
    """The <distribution>.buckify resolve targets in the active catalog cell."""
    out = subprocess.run(
        [buck2, "uquery", "kind('chroot_python_binary', catalog//...)"],
        check=True,
        capture_output=True,
        text=True,
    ).stdout
...
def _resolve(buck2: str, target: str, catalog_dir: Path) -> None:
...
def amalgamate(catalog_dir: Path) -> None:
    ...
    lines = [
        ...
        "# Amalgamation: loads each per-distribution JSON fragment's `value` (buck2 decodes",
...
def main(argv: list[str] | None = None) -> None:
    ...
    p.add_argument("--buck2", default="buck2", help="buck2 binary to nest (default: PATH)")
    p.add_argument(
        "--distribution",
        action="append",
    ...
    targets = _resolve_targets(args.buck2)
    ...
    for target in targets:
        _resolve(args.buck2, target, catalog_dir)
